...
import { filters, filtersKeys } from "../data/filters";
...
export class Player {
  ...
  public track_stop_reason: string | null = null;
  public filter: filters | null = null;
  ...
  public delete<T>(key: string): T {
    return delete this.data[key] as T;
  }
  ...
  public previous() {
    if (this.queue.previous.length != 0) {
      this.queue.unshift(this.queue.current);
      this.queue.unshift(this.queue.previous[this.queue.previous.length - 1]);
      this.queue.previous.pop();
      this.stop(undefined, 'PREVIOUS');
    };
   ...
   * setFilter
   */
  public setFilter(filter: filters | FilterBuilder | null) {
    if (!filter) {
      this.node.send({
        op: 'filters',
        guildId: this.guild
      ...
      if (!filtersKeys[filter]) throw new Error('Unknown filter.');
      this.node.send({
        op: 'filters',
        guildId: this.guild,
        ...filtersKeys[filter]
